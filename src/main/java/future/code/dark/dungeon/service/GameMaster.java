...
//        getStaticObjects().forEach(gameObject -> gameObject.render(graphics));
        getExitObjects().render(graphics);
        getCoinObject().forEach(gameObject -> gameObject.render(graphics));
...
//    }
    ...
    public GameObject getExitObjects(){
        return gameObjects.stream()
                .filter(gameObject -> gameObject instanceof Exit)
                .findFirst().orElseThrow();
    }

    public List<GameObject> getCoinObject(){
        return gameObjects.stream()
                .filter(gameObject -> gameObject instanceof Coin)
                .filter((gameObject -> !((Coin) gameObject).getState()))
                .collect(Collectors.toList());
    }
